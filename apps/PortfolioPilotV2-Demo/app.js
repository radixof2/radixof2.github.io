...
function recommendations() {
  ...
  const summary = `<div class="grid"><div class="card"><div class="label">Current-month STO cash received</div><div class="value">${money(R.current_month_sto_cash_received)}</div><p class="muted">Schwab opening-sale cash already received this month.</p></div><div class="card"><div class="label">Executable expected premium</div><div class="value">${money(R.expected_premium)}</div><p class="muted">Today’s qualifying recommendations only.</p></div><div class="card"><div class="label">Immediate target gap</div><div class="value">${money(R.target_gap)}</div><p class="muted">Target minus received cash and executable recommendations.</p></div><div class="card"><div class="label">Projected new premium through month end</div><div class="value">${money(R.projected_cadence_premium)}</div><p class="muted">Includes today’s executable recommendation plus illustrative later cycles.</p></div><div class="card"><div class="label">Projected month-end gap</div><div class="value">${money(R.projected_month_end_gap ?? R.target_gap)}</div><p class="muted">${esc(R.target_outlook || "Cadence outlook unavailable")}</p></div><div class="card"><div class="label">Policy mode</div><div class="value">Preserve shares</div><p class="muted">Read-only recommendations; constraints are never relaxed to meet target.</p></div></div><div class="warning-box"><b>Cadence outlook is illustrative, not guaranteed.</b> Projected new premium already includes today’s executable amount; do not add the two figures together. The outlook uses today’s qualifying option-chain premium and assumes replacement calls may be opened only after active calls expire and capacity becomes free. Future prices, deltas, liquidity, assignment, and available expirations will differ.</div>`;
  const blocks = symbols
    .map(
      (x) =>
        `<div class="section card"><div class="recommendation-heading"><div><h2>${esc(x.symbol)} recommendation</h2><p class="muted">${x.shares_owned.toLocaleString()} shares · Maximum ${x.max_active_contracts} active contracts · ${num(x.max_coverage_pct, 1)}% maximum coverage${x.earnings_event ? ` · Earnings ${esc(x.earnings_event.event_date)} (${esc(x.earnings_event.timing)}) · ${esc(x.earnings_event.source)} · ${x.earnings_event.authoritative ? "Issuer confirmed; expirations through that date excluded" : "Unverified discovery; not used to block recommendations"}` : ""}</p></div><span class="pill ${x.status === "READY" || x.status === "MONTHLY TARGET MET" ? "green" : "yellow"}">${esc(x.status)}</span></div><div class="grid mini-grid"><div><span class="label">Active</span><b>${x.active_contracts}</b></div><div><span class="label">Recycling soon</span><b>${x.recycling_contracts}</b></div><div><span class="label">Recommended now</span><b>${x.recommended_contracts}</b></div><div><span class="label">Expected now</span><b>${money(x.expected_premium)}</b></div><div><span class="label">Preferred cadence</span><b>${x.preferred_cadence_dte} DTE</b></div><div><span class="label">Weighted delta</span><b>${num(x.weighted_delta, 3)}</b></div></div>${table(
          [
            "Bucket",
            "Qty",
            "Expiration",
